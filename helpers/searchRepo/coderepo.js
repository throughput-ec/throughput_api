...
function searchRepo(req, res) {
  ...
  cypher_db    = "MATCH (n:OBJECT)-[:isType]-(:TYPE {type:'schema:CodeRepository'}) \
                  ...
                  RETURN DISTINCT n.name AS name, n.description AS description, n.url AS url, SIZE(COLLECT(o)) AS dbs \
                  ORDER BY dbs DESC \
                  SKIP {offset} \
                  LIMIT {limit}"
  ...
  cypher_db_kw = "MATCH (k:KEYWORD)-[]-(:ANNOTATION)-[]-(n:OBJECT)-[:isType]-(:TYPE {type:'schema:CodeRepository'}) \
                  ...
                  WITH DISTINCT n \
                  OPTIONAL MATCH (n)-[]-(:ANNOTATION)-[]-(o:OBJECT)-[:isType]-(:TYPE {type:'schema:DataCatalog'}) \
                  RETURN DISTINCT n.name AS name, n.description AS description, n.url AS url, toInteger(SIZE(COLLECT(o))) AS dbs \
                  ORDER BY dbs DESC \
                  SKIP {offset} \
                  LIMIT {limit}"
  ...
  .then(result => {
    ...
    if(count === 0) {
      ...
      .json({
        status: 'success',
        data: { count: 0, repos: null, database: null },
